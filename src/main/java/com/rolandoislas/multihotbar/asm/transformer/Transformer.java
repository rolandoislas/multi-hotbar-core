...
public class Transformer implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        ...
        if (doesAnyStringEqual(className, name, transformedName)) {
            ...
            try {
                ...
                transformMethodBipush("getCurrentItem", "()Lnet/minecraft/item/ItemStack;",
                        "h", "()Ladd;",
                        VANILLA_HOTBAR_SIZE, HOTBAR_SIZE, classNode);
            }
    ...
    /**
     * Transform Minecraft's middleClickMouse offset. It only expects 0-8 as the current item.
     * This patch replaces the offset with a static method call that returns the correct offset.
     * @see Transformer#fixOffsetPickBlock(int)
     * @param classNode node in which to find method
     */
    private void transformPickBlock(ClassNode classNode) {
        String name = "func_147112_ai";
        String nameObf = "ao";
        String desc = "()V";
        String descObf = "()V";
        ...
        for (MethodNode method : classNode.methods) {
            if ((method.name.equals(name) || method.name.equals(nameObf)) &&
                ...
                while (instructionIterator.hasNext()) {
                    AbstractInsnNode instruction = instructionIterator.next();
                    if (instruction instanceof VarInsnNode && instruction.getOpcode() == Opcodes.ALOAD &&
                            ((VarInsnNode)instruction).var == 0 && label == 5) {
                        found = true;
                        // Add nodes that calculate the offset to a list of nodes to remove
                        List<AbstractInsnNode> toRemove = new ArrayList<>();
                        toRemove.add(instruction);
                        toRemove.add(method.instructions.get(method.instructions.indexOf(instruction) + 11));
                        for (int instructionAfter = 1; instructionAfter <= 6; instructionAfter++) {
                            toRemove.add(method.instructions.get(method.instructions.indexOf(instruction) +
                                    instructionAfter));
                        }
                        // Insert method call
                        AbstractInsnNode currentItem = method.instructions.get(
                                method.instructions.indexOf(instruction) + 10);
                        method.instructions.insert(currentItem, new MethodInsnNode(
                                Opcodes.INVOKESTATIC,
                                "com/rolandoislas/multihotbar/asm/transformer/Transformer",
                                "fixOffsetPickBlock", "(I)I", false
                        ));
                        // Removes nodes that were pending removal
